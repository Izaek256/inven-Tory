/**
 * LoginForm — Web dashboard login component.
 *
 * Issue 25 — Auth consolidation.
 * Authenticates against the central /api/v1/auth/login endpoint.
 * Web uses standard JWT login (no device verification needed for web).
 */
...
export const LoginForm: React.FC<LoginFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    setLoading(true);
    try {
      // Use FastAPI Users standard JWT login for web
    ...
    <form onSubmit={handleSubmit} data-testid="login-form">
      <div style={{ marginBottom: '16px' }}>
        <TextInput
          ...
          disabled={loading}
          data-testid="web-login-email-input"
        />
      </div>
      ...
      <div style={{ marginBottom: '24px' }}>
        <TextInput
          ...
          disabled={loading}
          data-testid="web-login-password-input"
        />
      </div>
      ...
      <Button
        ...
        loading={loading}
        style={{ width: '100%' }}
        data-testid="web-login-submit-btn"
      >
        <LogIn size={18} />
        <span>Sign In</span>
      </Button>
