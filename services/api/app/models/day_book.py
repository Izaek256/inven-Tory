...
class DayBook(Base):
    ...
    balance_sheet_generated_at: Mapped[datetime | None] = mapped_column(
    ...
    __table_args__ = (Index("idx_day_books_store_date", "store_id", "book_date"),)
...
class DayBookEntry(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    day_book_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("day_books.id"), nullable=False, index=True
    )

    # Reference to the original transaction
    transaction_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("inventory_transactions.transaction_id"), nullable=False, index=True
    )

    # Entry details
    movement_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # SALE, RECEIPT, RETURN, etc.
